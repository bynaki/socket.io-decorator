...
test.cb('ack', t => {
  const socket = Socket('http://localhost:8110/mock')
  socket.emit(':ack', 'Hello World!!', (msg: string) => {
...
test.cb('ack', t => {
  const socket = Socket('http://localhost:8110/mock')
  socket.emit(':ack.async', 'Hello World!!', (msg: string) => {
    t.is(msg, 'Hello World!!')
    socket.close()
    t.end()
  })
...
test.cb('auth > on and off level01', t => {
  ...
  socket.emit(':auth.level01', 'on', async (msg: string) => {
    t.is(msg, 'ok')
    ...
    socket.on(':auth.level01', res => {
      t.is(res, 'level01')
      socket.emit(':auth.level01', 'off', async (msg: string) => {
        t.is(msg, 'ok')
        const level01Room = mock.namespace.in('level01')
        const clients: string[] = await p(level01Room.clients, level01Room)()
...
test.cb('auth > on and off level02', t => {
  ...
  socket.emit(':auth.level02', 'on', async (msg: string) => {
    t.is(msg, 'ok')
  ...
  socket.on(':auth.level02', res => {
    t.is(res, 'level02')
    socket.emit(':auth.level02', 'off', async (msg: string) => {
      t.is(msg, 'ok')
      const level02Room = mock.namespace.in('level02')
      const clients: string[] = await p(level02Room.clients, level02Room)()
      t.is(clients.indexOf(socket.id), -1)
...
test.cb('@Use > order', t => {
  const socket = Socket('http://localhost:8110/mock')
  socket.emit(':use.order', (...track: string[]) => {
...
test.cb('error > in @On()', t => {
  const socket = Socket('http://localhost:8110/mock')
  socket.emit(':ack', 'error', (msg: string) => {
...
test.cb('error > in @On() async', t => {
  const socket = Socket('http://localhost:8110/mock')
  socket.emit(':ack.async', 'error', (msg: string) => {
    t.is(msg, 'error: error')
    socket.close()
    t.end()
  })
